...
inspect artifact, audit, backup, and blocking-reason checks. It can also load
...
The smoke also starts the installed editor and requires its local Workflow DSL
validation route to return the fixed valid, value-free result for the copied
public fixture.
...
For interactive local `SKILL.md` compilation, use the installed UI server
instead of a generic static server:

```bash
skill2workflow ui --port 4173
```

Choose **SKILL.md** and then **Compile SKILL**. This creates only an in-memory
draft DSL for inspection; it does not write, publish, execute, or access a
service token. See [authoring guidance](docs/authoring.md).

In the installed UI, use **Validate DSL** before **Save DSL**. It invokes the
same local compiler validator used by the CLI and returns only bounded,
source-free issue codes and safe node ordinals; a valid draft can then be
downloaded as Workflow DSL.
...
Inspect the execution plan before triggering a local workflow:
...
For a published version on a self-hosted service, use the authenticated
`service-workflow-explain` command. The plan shows topology, human gates,
connector metadata, input-contract shape, retries, and timeouts without
...
Create a fresh, owner-only, bounded redacted evidence page only after the
complete SQLite audit chain verifies:

```bash
PYTHONPATH=src python3 -m skill2workflow.cli audit-evidence \
...
Current maturity: Self-hosted Beta. The local-first harness covers all five approved architecture layers, and Delivery Loops 1-262 are complete.
...
schedules. Exact origins are checked before credentials or network access;
omission preserves Workflow DSL compatibility. See the [service configuration
boundary](docs/service-config-boundary.md) and [connector guide](docs/connectors.md).

Loop 201 makes that boundary discoverable during first-run setup: repeat
`service-init --http-allowed-origin ORIGIN` for each approved destination, and
the initializer validates and canonicalizes the values before creating the
workspace.

Loop 202 keeps external connector provider details out of durable failure
...
Loop 204 lets an explicitly loaded external connector declare an additional
...
Loop 208 adds a fixed service-readiness badge to that live console. It reports
`ready`, `not ready`, or `unavailable` through the existing health/readiness
probe contract, without exposing credentials or adding an arbitrary proxy.

Loop 209 adds explicit 10-second live snapshot refresh with visibility-aware
pausing and stale-data preservation on transient failures; static mode remains
poll-free.

Loop 210 adds one-click download of the existing bounded, redacted support
bundle from the live console. The fixed route remains read-only and server-side
authenticated; no automatic upload or arbitrary proxy is introduced.

Loop 214 adds a confirmation-protected **Cancel run** action to the live console
...
Loop 216 adds bounded live recurring-schedule discovery to the same console.
...
Loop 228 closes the remaining safe live-console handoff for no-input workflows:
after a just-loaded ready empty preflight, a published exact version can start
only through an explicit confirmation and a browser-generated idempotency key.
...
Loop 231 adds a direct authoring entry point: the loopback installed editor can
...
Loop 237 closes the visual-authoring validation gap: installed **Validate DSL**
and **Save DSL** call a bounded local compiler-validation route before a DSL
download. Invalid documents return source-free issue codes only, rather than
author-entered text or runtime data.

Loop 238 makes those source-free validation findings actionable in the editor:
...
Loop 240 makes that local handoff independently verifiable: `authoring-verify`
...
Loop 242 moves the conservative secret-hygiene boundary ahead of local author
artifact writes and repeats it on verification. An obvious secret-like value
now produces no authoring directory and never appears in a verification result.

Loop 243 keeps authoring provenance useful without leaking local filesystem
locations: exported DSL uses the fixed `SKILL.md` source marker, while portable
Bundle creation and verification reject absolute local source paths.

Loop 244 makes documented first-value paths release-relevant CI evidence: a
separate Python 3.14 job runs the local Pilot, scenario pack, explicit external
connector fixture, and isolated installed-wheel quickstart without live
providers or credentials.

Loop 245 adds the missing authoring-to-runtime proof to that job: a local
...
Loop 247 closes the authoring recovery gap: after verification detects a
damaged local authoring set, `authoring-repair` rebuilds a fully verified
replacement from the original Skill and preserves the prior set at an explicit
sibling backup path. Publication and execution remain separate actions.

Loop 248 makes recovery part of the complete delivery proof: the local
Skill-to-runtime smoke deliberately detects a damaged set, repairs it, then
continues through verified Bundling, publication, an approved run, and a
separately rejected run with audit evidence.

Loop 249 adds `authoring-repair --dry-run`, which fully rebuilds and verifies
a temporary candidate but leaves the selected authoring and backup paths
unchanged. Operators can therefore review a value-free readiness result before
the explicit repair swap.

Loop 250 adds [`audit-evidence`](docs/audit-evidence.md), a deliberately
...
Loop 256 brings that target review into the protected live console. A staged
workflow must first pass structural release preflight, then show the operator a
...
Loop 258 applies the same recovery rule to production-alias promotion and
version deprecation: a compare-and-swap `409` disables the stale action until
the operator explicitly reloads the protected workflow inventory. The UI never
retries or assumes that either mutation succeeded.
...
Loop 155 adds [protected remote recurring-schedule updates](docs/remote-schedule-update.md):
the authenticated service and installed `service-recurring-schedule-update`
client require the last observed `next_run_at` compare-and-swap token, preserve
...
Loop 161 adds [cursor-paged remote backup inventory](docs/remote-backup-inventory-pages.md):
the separate authenticated page route and installed
`service-backup-inventory-page` client walk older redacted backup evidence with
...
Loop 163 bounds the same [remote backup retention plan](docs/remote-backup-retention-plan.md)
at the filesystem boundary: once the fixed 1,000-set budget is exceeded, the
preflight stops after the first over-budget directory and fails closed without
changing the existing redacted response contract.

Loop 164 adds lazy bounded discovery for local one-shot schedules: bounded due
...
Loop 170 hardens the dependency-light JSON run-state path: each serialized
state is capped at 8 MiB, and load, listing, and interrupted-run recovery use
regular-file/no-follow descriptor reads bound to one device/inode with a
growth-race recheck. The JSON storage contract remains compatible; the
...
Loop 182 adds value-free [bundle structural diff review](docs/workflow-bundles.md):
`bundle-diff` verifies both artifacts and reuses the published `workflow-diff`
semantics before reporting only changed sections and node/edge IDs.
...
Loop 185 adds an explicit [Bundle side-effect consent](docs/workflow-bundles.md)
