...
- HTTP method;
- normalized route class (`workflow_trigger`, `run_resume`, `run_cancel`, `run_list`, `run_page`, `audit_event_list`, `run_detail`, `audit_consistency`, `recurring_schedule_create`, `recurring_schedule_action`, `recurring_schedule_update`, `recurring_schedule_delete`, `support_bundle`, or `unknown`);
- timestamp;
