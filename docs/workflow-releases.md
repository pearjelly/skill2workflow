...
This is an operator review aid and a local optimistic-concurrency guard. It is
not a policy engine, approval workflow, semantic business-risk analyzer,
automatic canary, health-based rollout, rollback controller, signature, or
multi-tenant release service. Operators remain responsible for reviewing the
published diff and choosing the target version.
