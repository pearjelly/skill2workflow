...
The only HTTP route labels are `health`, `readiness`, `metrics`, `control_snapshot`, `recurring_schedule_list`, `recurring_schedule_action`, `audit_consistency`, `support_bundle`, `run_list`, `run_detail`, `workflow_trigger`, `run_resume`, `run_cancel`, and `unknown`. The only response-class labels are `2xx`, `4xx`, and `5xx`. Unknown persisted statuses roll into `other`; user-controlled values never become label values. This fixed matrix intentionally favors safe alerting and bounded time-series count over per-workflow diagnosis.
