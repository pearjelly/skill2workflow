...
| `POST /api/v1/workflow-release-target-reviews` | Requires readiness and Bearer authentication with exactly `{"workflow": <object>}` within the 1 MiB request bound. Reads the matching immutable target and returns only candidate/existing recognition digests plus `new`, `idempotent`, or `conflict` and the existing value-free preflight projection. It does not write registry or audit state, reserve the version, resolve credentials, or invoke connectors. The result is advisory at one instant; immutable publication remains the authoritative race-safe decision. The installed `service-workflow-release-target-review` and verified-set `authoring-service-release-target-review` clients wrap this route. See [`remote-workflow-release.md`](remote-workflow-release.md). |
