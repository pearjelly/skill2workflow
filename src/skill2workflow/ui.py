    ...
    *,
    ...
    class QuietHandler(SimpleHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if parsed.path == _WORKFLOW_EMPTY_TRIGGER_PATH:
                if parsed.query:
                    self._write_json(
                        404,
                        {"error": "workflow empty trigger path is not available"},
                    )
                    return
                self._serve_live_workflow_empty_trigger()
                return
        ...
        def _serve_live_workflow_empty_trigger(self):
            ...
            token_file = getattr(self.server, "live_auth_token_file", None)
            if configured_service_url is None or token_file is None:
                self._write_json(404, {"error": "workflow empty trigger is not configured"})
                return
            if self.headers.get("Transfer-Encoding"):
                self._write_json(400, {"error": "workflow empty trigger body is malformed"})
                return
            content_lengths = self.headers.get_all("Content-Length", [])
            if len(content_lengths) != 1:
                self._write_json(400, {"error": "workflow empty trigger body is malformed"})
            ...
            if content_length < 0:
                self._write_json(400, {"error": "workflow empty trigger body is malformed"})
                return
            if content_length > _WORKFLOW_EMPTY_TRIGGER_MAX_REQUEST_BYTES:
                self._write_json(413, {"error": "workflow empty trigger body is too large"})
                return
            body = self.rfile.read(content_length)
            if len(body) != content_length:
                self._write_json(400, {"error": "workflow empty trigger body is malformed"})
            ...
            if (
                not isinstance(payload, dict)
                or set(payload) != {"workflow_id", "version", "idempotency_key"}
                or not _is_safe_workflow_ref(payload.get("workflow_id"), allow_empty=False)
                or not _is_safe_workflow_ref(payload.get("version"), allow_empty=False)
            ...
            try:
                ...
                if len(response_body) > _WORKFLOW_EMPTY_TRIGGER_MAX_RESPONSE_BYTES:
                    raise ValueError("workflow empty trigger unavailable")
            except ServiceActionError as error:
                if error.status_code == 400:
                    self._write_json(400, {"error": "workflow empty trigger rejected"})
                elif error.status_code == 404:
                    self._write_json(404, {"error": "workflow version not found"})
                elif error.status_code == 409:
                    self._write_json(409, {"error": "workflow empty trigger idempotency conflict"})
                elif error.status_code == 413:
                    self._write_json(413, {"error": "workflow empty trigger body is too large"})
                else:
                    self._write_json(503, {"error": "workflow empty trigger unavailable"})
                return
            except (TypeError, ValueError):
                self._write_json(400, {"error": "workflow empty trigger body is malformed"})
                return
