...
class ControlPlaneTests(TestCase):
    ...
    def test_sqlite_alias_resolution_does_not_load_global_registry(self):
        with TemporaryDirectory() as tmp:
            control = LocalControlPlane(Path(tmp), storage="sqlite")
            control.publish_workflow(_workflow(version="1.0.0"))
            ...
            with patch.object(
                control,
                "_load_index",
                side_effect=AssertionError("trigger resolution loaded the global registry"),
            ):
                resolved = control.resolve_workflow_version(
                    "workflow_control", "production"
                )
    ...
    def test_node_timeout_terminal_audit_contains_fixed_error_code(self):
        clock = _TestClock()
        workflow = _connector_workflow("8.0.0", "https://unused.invalid")
        ...
        self.assertEqual(state["status"], "failed")
        self.assertEqual(events[-1]["type"], "run_failed")
        self.assertEqual(events[-1]["error_code"], "node_timeout")
    ...
    def test_recurring_schedule_delete_audit_is_bounded_and_allowlisted(self):
        with TemporaryDirectory() as tmp:
            control = LocalControlPlane(Path(tmp), storage="sqlite")
            control.record_recurring_schedule_deleted(
                "schedule_hourly_report",
                deleted=False,
            )
            event = control.list_audit_events()[0]
        ...
        self.assertEqual(
            set(event), {"type", "schedule_id", "deleted", "timestamp"}
        )
        self.assertEqual(event["type"], "recurring_schedule_deleted")
        self.assertEqual(event["schedule_id"], "schedule_hourly_report")
