...
class UiTests(TestCase):
    ...
    def test_live_proxy_reports_run_action_conflicts_without_browser_token(self):
        cases = (
            ("/api/v1/runs/run_waiting/resume", b'{"approved":true}', {"error": "run is not waiting"}),
            ("/api/v1/runs/run_waiting/cancel", b"{}", {"error": "run cannot be cancelled"}),
        )
        for path, request_body, expected_body in cases:
            with self.subTest(path=path):
                observed = {}
                ...
                class UpstreamHandler(BaseHTTPRequestHandler):
                    def do_POST(self):
                        ...
                        body = b'{"error":"upstream conflict"}'
                ...
                try:
                    with tempfile.TemporaryDirectory() as directory:
                        ...
                        self.assertIn("value", ui_port)
                        request = urllib.request.Request(
                            f"http://127.0.0.1:{ui_port['value']}{path}",
                            data=request_body,
                            headers={"Content-Type": "application/json"},
                            method="POST",
                        )
                        ...
                        try:
                            ...
                            self.assertEqual(
                                json.loads(error.read().decode("utf-8")), expected_body
                            )
                        finally:
                            error.close()
                        ui_thread.join(timeout=2)
                        ...
                        self.assertEqual(
                            observed["path"],
                            "/runs/run_waiting/" + ("resume" if path.endswith("resume") else "cancel"),
                        )
                        self.assertEqual(observed["body"], request_body)
                        self.assertEqual(
                            observed["authorization"],
                            "Bearer ui-test-token-012345678901234567890123456789",
                        )
    ...
    def test_live_proxy_reports_schedule_dispatch_review_conflict_without_browser_token(self):
        observed = {}
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                try:
                    ...
                    self.assertEqual(
                        json.loads(error.read().decode("utf-8")),
                        {"error": "dispatch review conflict"},
                    )
                finally:
                    error.close()
                ui_thread.join(timeout=2)
    ...
    def test_live_proxy_reports_workflow_mutation_conflicts_without_browser_token(self):
        cases = (
            (
                ...
                {"error": "workflow promotion conflict"},
            ),
            (
                ...
                {"error": "workflow deprecation conflict"},
            ),
        )
