...
class GoLiveDocumentationTests(TestCase):
    def test_go_live_checklist_links_existing_safe_operator_controls(self):
        ...
        self.assertIn("### 2a. Preflight a configured Feishu China credential", guide)
        self.assertIn("skill2workflow service-lark-tenant-credential-check", guide)
        ...
        self.assertIn("does not start the service, publish or start a", guide)
        self.assertIn("remain local/provider-free", guide)
        self.assertIn("short-circuits before the service Probe", guide)
