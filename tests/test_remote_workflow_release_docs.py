...
class RemoteWorkflowReleaseDocumentationTests(TestCase):
    def test_remote_publication_contract_is_published_across_project_surfaces(self):
        ...
        self.assertIn("service-workflow-release-target-review", guide)
        self.assertIn("authoring-service-publish", guide)
        self.assertIn("authoring-service-release-preflight", guide)
        self.assertIn("authoring-service-release-target-review", guide)
        self.assertIn("workflow-release-preflights", guide)
        self.assertIn("workflow-release-target-reviews", guide)
        self.assertIn("point-in-time", guide)
        self.assertIn("not a lock", guide)
        ...
        self.assertIn('"service-workflow-release-target-review"', package_smoke)
